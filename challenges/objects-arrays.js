...
let uni = graduates.map(graduates => graduates.university);
const universities = uni.sort();
...
const unisWithUni = [];
let includes = function(uniArray){
for (let i = 0; i <uniArray.length; i++){
  if (uniArray[i].includes('Uni') === true){
    unisWithUni.push(uniArray[i]);
    }
  }
}
includes(uni);
...
*/
let pops = zooAnimals.map(item => item.population) 
const reducer = (total, num) => total + num;
const populationTotal = pops.reduce(reducer)
